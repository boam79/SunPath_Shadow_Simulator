...
| 항목 | 내용 |
|---|---|
| **Sentry 에러 모니터링** | `@sentry/nextjs` 설치. `instrumentation.ts`(서버) + `instrumentation-client.ts`(클라이언트). Vercel에 `NEXT_PUBLIC_SENTRY_DSN` 환경변수 추가 시 자동 활성화 |

#### ⚙️ 활성화 필요 항목 (Vercel 환경변수)

```
NEXT_PUBLIC_SENTRY_DSN=https://xxxx@o0.ingest.sentry.io/0  # Sentry DSN
```

#### 💡 UptimeRobot 콜드스타트 방지 (무료)
1. [UptimeRobot](https://uptimerobot.com) 가입
2. New Monitor → HTTP(s) → `https://sunpath-shadow-simulator.onrender.com/health`
3. 모니터링 간격: 5분
4. → Render Free 인스턴스가 슬립에 들어가지 않아 콜드스타트 50초 문제 해소
