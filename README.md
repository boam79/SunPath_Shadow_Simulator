...
### 7) 기능 고도화 로드맵+태스크
- [x] ✅ 지도 범례/표기: 빨간 핀(기준점), 노란 점선(그림자 궤적) 등 의미를 지도 우측 하단에 간단히 표기
- [x] ✅ 일사량/고도 기반 컬러 그라데이션: 타임라인 슬라이더와 맵 선 색상 연동
- [x] ✅ 차트: 시간대별 고도/방위/일사량 3축 요약 차트
- [ ] 사용자 프리셋 저장/불러오기(LocalStorage)
- [x] ✅ 다국어(i18n): ko/en 토글 (v0.1.7)
...
### Version 0.1.7 (2025-11-02)

**다국어 지원 및 SEO 최적화 - 글로벌 사용자 경험 향상**

#### 🌐 주요 기능
- **다국어 지원 (i18n)**
  - Custom Context API 기반 i18n 구현
  - 한국어(ko) / 영어(en) 언어 전환
  - 모든 UI 컴포넌트 다국어 적용
...
- **OptimizationPanel 에러 메시지 수정** (`frontend/components/OptimizationPanel.tsx`)
