...
```bash
cd backend
# Python 3.11 권장 (로컬에 venv 후 pip install -r requirements.txt)
pytest tests/ -v
```
...
npm run lint
npm run build
npm run test:e2e   # Playwright (빌드 후 next start 자동 기동)
```

## 📊 API 엔드포인트

**버전 경로:** 권장 프리픽스는 `/api/v1/...` 입니다. 기존 `/api/...` 도 동일 라우터로 제공됩니다.
...
**Solar Position:** (`/api/v1/solar/...` 또는 `/api/solar/...`)
- `POST /api/v1/solar/position` - 태양 위치 시계열 계산
- `GET /api/v1/solar/sunrise-sunset` - 일출/일몰 시각
- `GET /api/v1/solar/test` - 정확도 검증

**Shadow:**
- `GET /api/v1/shadow/calculate` - 그림자 계산
- `GET /api/v1/shadow/test` - 테스트
- `GET /api/v1/shadow/validate` - 검증

**Irradiance:**
- `GET /api/v1/irradiance/calculate` - 일사량 계산 (Perez Sky Model 지원 ✅ NEW)
- `GET /api/v1/irradiance/test` - 테스트
- `GET /api/v1/irradiance/sunrise-sunset-irradiance` - 일출/일몰 일사량

**Integrated:**
- `POST /api/v1/integrated/calculate` - 통합 계산 ⭐ (프론트엔드 사용)
- `POST /api/v1/integrated/batch` - 배치 계산 (여러 위치/날짜 동시 처리) ✅ NEW

**Cache:**
- `GET /api/v1/cache/stats` - 캐시 통계
- `POST /api/v1/cache/clear` - 캐시 삭제
- `GET /api/v1/cache/test` - 캐시 성능 테스트

**API 문서:** 개발 환경에서 `http://localhost:8000/docs` — 프로덕션(`ENVIRONMENT=production`)에서는 기본 비활성화. 필요 시 Render에 `SUNPATH_ENABLE_DOCS=true` 설정.
...
**Render 무료 플랜 특이사항:**
- ⚠️ 15분 비활성화 시 슬립 모드 (첫 요청 시 콜드 스타트 가능). 유료 플랜으로 상시 기동 가능
...
- **기상 API 수정**: Open-Meteo는 `start_date`/`end_date`와 `forecast_days`를 동시에 받을 수 없어 `/api/weather`가 502를 내는 문제를 수정했습니다.
- **CSP**: MapLibre/OSM 타일 로드를 위해 `connect-src`·`img-src`에 OpenStreetMap 호스트를 추가했습니다.
- **E2E**: `/api/weather`, `/api/geocode`, `/api/backend` 헬스·통합 계산 프록시 테스트 추가.

### v0.1.17

> **작업일:** 2026-04-05

- **품질**: `backend/tests` pytest + GitHub Actions CI(백엔드·프론트 lint/build/Playwright). 요청 로깅·`X-Request-ID`, GZip, `/api/*` IP당 슬라이딩 윈도 레이트리밋(180/분)
- **기능**: API 클라이언트를 `/api/v1`으로 정렬(백엔드는 `/api` 별칭 유지). 사이드바 **기상 요약(WeatherStrip)**, **두 물체 높이 비교**, **요약 인쇄/PDF** 버튼. 계산 후 `localStorage` 요약 백업
- **보안·안정**: CORS 메서드/헤더 축소, 프로덕션에서 OpenAPI 기본 비활성화(`ENVIRONMENT=production`, `render.yaml` 반영). Next **CSP** 헤더. `/api/geocode`·`/api/weather` 입력 검증

### v0.1.16

> **작업일:** 2026-04-05

- **Sentry 제거**: `@sentry/nextjs` 및 `instrumentation*.ts` 삭제, `next.config.mjs`에서 Sentry 래퍼 제거
- **UptimeRobot 문서 정리**: 외부 핑 모니터링 가이드 삭제(프로젝트에서 추적하지 않음)

### v0.1.15

> **작업일:** 2026-04-04 · **커밋:** `becc756`

#### 🟢 P0 — 안정성

| 항목 | 내용 |
|---|---|
| **Header 실제 헬스체크** | 초록(정상) / 노랑(콜드스타트) / 빨강(오류) / 회색(확인중) 4단계 상태 표시. 5분마다 `/api/backend/health` 실제 호출. 클릭으로 즉시 재확인 가능 |

#### 🟠 P1 — 핵심 UX

| 항목 | 내용 |
|---|---|
| **콜드스타트 감지 UI** | 요청 5초 초과 시 "서버가 깨어나는 중… (XX초 경과)" 배너 자동 표시 |
| **URL 파라미터 공유** | `lat/lon/date/height` 쿼리스트링 자동 동기화. 푸터 "현재 위치 공유" 버튼으로 클립보드 복사 |
| **sessionStorage 캐싱** | 같은 위치·날짜·높이 30분 내 재요청 시 캐시 반환 (불필요한 Render 호출 제거) |
| **타임라인 범위 동적 조정** | 하드코딩 05:00~20:00 → 일출 -1h ~ 일몰 +1h 자동 계산 |
| **지오코딩 서버사이드 프록시** | `/api/geocode` 추가. 서버 IP로 Nominatim 호출 → IP 차단 위험 감소. 한국 제한 제거 → 전 세계 주소 검색. Vercel CDN 1시간 캐시 |

#### 🟡 P2 — 기능 확장

| 항목 | 내용 |
|---|---|
| **Open-Meteo 기상 연동** | `/api/weather` 라우트 추가. 구름량·일사량·강수확률 시간별 데이터 제공 (`lib/weather.ts`) |
| **PWA 지원** | `public/manifest.json` 추가, `layout.tsx`에 manifest 메타 연결. 모바일 홈 화면 추가 가능 |
...
- **AWS EC2 비가동 확인**: `54.180.251.93` 타임아웃 → 더 이상 사용 불가
- **Render 백엔드 정상 확인**: `https://sunpath-shadow-simulator.onrender.com/health` → HTTP 200

#### 🔧 GitHub Actions 수정

- `deploy-backend.yml`: `push` 트리거 제거 → `workflow_dispatch`(수동) 전용으로 변경
  - 이전: `master` 푸시마다 비가동 AWS EC2 배포 시도 → 매번 실패
  - 이후: EC2 복구 시에만 수동 실행
- `scripts/check-backend-production.sh` 추가: Render + 레거시 AWS 헬스체크 통합 스크립트

#### 🛠 CORS 문제 근본 해결

**원인 분석 (3단계):**
1. `ALLOWED_ORIGINS = https://sunpath-simulator.vercel.app` → 실제 도메인과 다름 (`sunpathshadowsimulator`)
2. Render가 OPTIONS preflight에 HTTP 405 반환 → 브라우저 CORS 차단 → `Failed to fetch`
3. 구버전 `api.ts`: HTTPS 백엔드는 프록시 미경유 → 브라우저가 Render 직접 호출 → CORS 차단

**적용한 수정:**

| 파일 | 변경 내용 |
|---|---|
| `render.yaml` | `ALLOWED_ORIGINS` → `https://sunpathshadowsimulator.vercel.app` 정렬 |
| `RENDER_ENV_VARS.md` | 도메인 오타 수정 |
| `frontend/lib/api.ts` | 출처가 다르면(HTTPS 포함) 항상 프록시 경유로 변경 |
| `frontend/app/api/proxy/[...path]/route.ts` | `BACKEND_URL` 폴백: `BACKEND_API_URL → NEXT_PUBLIC_API_URL → Render URL` |
| `frontend/next.config.mjs` | `NEXT_PUBLIC_API_URL` 미설정 시 `localhost` 대신 Render URL 기본값 |

#### 🔄 Next.js Rewrite로 프록시 전환 (최종 해결)

커스텀 API Route(`/api/proxy`) 대신 **Next.js 빌트인 rewrite**(`/api/backend`) 사용:

```js
// next.config.mjs
rewrites: [{ source: '/api/backend/:path*', destination: 'https://...onrender.com/:path*' }]
```

- 브라우저 → `/api/backend/*` (동일 출처) → Vercel 인프라가 서버사이드로 Render 전달
- CORS preflight 발생하지 않음 (동일 출처 요청)
- 커스텀 Node.js 프록시 코드 불필요
...
|---|---|
| `NEXT_PUBLIC_API_URL` | `https://sunpath-shadow-simulator.onrender.com` |
| `BACKEND_API_URL` | `https://sunpath-shadow-simulator.onrender.com` |

#### 주요 커밋

| 커밋 | 내용 |
|---|---|
| `7ebaa4a` | Render 프로덕션 고정, AWS Actions 수동 전환 |
| `0e2d5e2` | HTTPS 백엔드도 프록시 경유하도록 api.ts 수정 |
| `b9e5a89` | 프록시 route.ts Render URL 기본값 설정 |
| `d0495b8` | Next.js rewrite 도입, /api/backend 경로 사용 |
