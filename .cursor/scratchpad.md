...
- [x] **다중 건물 레이캐스트** (PR #11 → master 머지 완료 `59cc696`)
...
**머지:** PR #11 → `master` fast-forward (`59cc696`) 완료. Vercel production 재배포 대기·확인.

**사용자 확인 요청:** production에서 레이캐스트 ON → 주변 건물 그림자·물체 그림자 단축·음영 배너.
...
**방향:** Planner S1 / D1 승인(「d1으로 맞춰」).

**변경:**
- 탭 3개: 시뮬레이트 | 비교 | 도구
- 시뮬레이트: 검색+GPS+퀵시티, 좌표 접기, 날짜, 타임라인, 높이 슬라이더만 (`type=time`·중복 number·현재설정 카드 제거)
- 비교: 높이 비교 + 계절 + 배치
- 도구: 기상·보내기·프리셋·고급·후원 1회
- 사이드바 상·하 카카오페이 중복 제거
- 사이드바 열 flex 높이 체인

**사용자 확인:** PC에서 시뮬레이트 탭이 스크롤 없이 핵심 조작이 보이는지.


### 2026-07-23 - Planner: 왼쪽 사이드바 UX 제안 (구현 전 승인 대기)

**역할:** Planner — 분석·제안만. 코드 변경 없음.

**현재 문제 요약 (SingleTabPanel + Sidebar 정적 분석 + PC 스크린샷):**
1. **정보 과밀:** 하루 보기 탭에 위치·위경도·GPS·퀵시티·날짜·시즌칩·타임라인·높이 슬라이더+숫자·비교·time input·현재설정 카드·기상·보내기 5버튼이 한 스크롤에 몰림. 720p에서 타임라인이 잘림.
2. **후원 CTA 중복:** 사이드바 상단+하단 + footer에 카카오페이 — 제어 패널이 광고판처럼 보임.
3. **탭 라벨 모호:** 「하루 보기 / 여러 번 / 계절 비교 / 더보기」— 일반 사용자에게 모드 의미가 불명확. 2×2 그리드가 주 작업(위치·시간)보다 시각적으로 먼저 옴.
4. **입력 중복:** 타임라인 ↔ `type=time` 이중; 높이 슬라이더 ↔ number 이중; 위경도 상시 노출은 전문가만 필요.
5. **역할 혼선:** 사이드바=조작, 지도 HUD=결과인데 「현재 설정」카드가 결과/요약을 다시 반복.
6. **보내기 구역:** 색색 그라데이션 버튼 5개가 주 플로우 아래를 잡아먹음.

**권장 방향 S1 — Instrument Console (D1과 정합, 권장)**
- 사이드바 상단 고정: 검색 + GPS 아이콘 + 서울/부산/제주 칩만 (위경도는 「좌표 직접 입력」접이식)
- 그다음: 날짜 + 시즌 칩, 타임라인(재생 포함). `type=time` 제거(타임라인과 통합)
- 높이: 슬라이더 하나 + 값 텍스트. number는 고급에만
- 비교·기상·보내기·프리셋·배치·계절: 「도구」또는 접이식 섹션으로 강등
- 탭: `시뮬레이트 | 비교 | 도구` 3탭 또는 탭 제거하고 accordion
- 후원: 사이드바에서 제거하고 footer/더보기에만 1회
- sticky 하단: 재생 컨트롤만 (스크롤해도 항상 보임)

**대안 S2 — Minimal + Overflow:** 핵심 4블록만 남기고 나머지 전부 「더 보기」시트.
**대안 S3 — 섹션 아코디언:** 탭 유지하되 single 내부를 접이식 섹션화(덜 급진적).

**성공 기준(구현 시):**
- 1440×900 / 1280×720에서 스크롤 없이 위치·날짜·타임라인·높이가 보임
- 카카오페이 사이드바 중복 제거
- 위경도/보내기/비교는 1클릭 이내이되 기본 화면을 어지럽히지 않음

**사용자 확인 필요:** S1 / S2 / S3 중 선택 + Executor 진행 여부
